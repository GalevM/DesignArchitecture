...
# Отвори CSV датотека за пишување
with open('scraped_data_sentiment.csv', mode='w', newline='', encoding='utf-8') as csvfile:
    csvwriter = csv.writer(csvfile)
    # Запиши го заглавието
    csvwriter.writerow(['Issuer', 'Title', 'Content'])

    # Функција за скрепување на еден издавач
    def scrape_issuer(issuer):
        driver.get(base_url + issuer)
        time.sleep(3)  # Почекај елементите да се вчитаат
        ...
        # Наоѓање на линковите под селекторот "container-seinet"
        try:
        ...
        for item in news_items:
            ...
            try:
                # Скрепување на содржината од целниот div
            ...
            # Врати се назад на главната страница
            driver.back()
            time.sleep(2)
