...
def page5():
    ...
    # Path to the pre-saved results and graph
    results_file = 'predictions.csv'  # Path to your saved results file
    graph_path = 'static/lstm_predictions.png'  # Path to your saved graph image

    # Check if the LSTM results and graph already exist
    if os.path.exists(results_file) and os.path.exists(graph_path):
        # If files exist, load and display the pre-saved results and graph
        with open(results_file, 'r') as file:
            results = file.readlines()
            rmse = results[0].strip()  # Assuming RMSE is the first line
            predictions = results[1:]  # Assuming predictions are the following lines

        graph_url = graph_path  # Path to the saved graph image
    else:
        # If the results or graph don't exist, show an error message or a placeholder
